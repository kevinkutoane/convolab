...
import {
  AuthApiError,
  breakGlassLogin as breakGlassLoginRequest,
  getSession,
  login as loginRequest,
  logout as logoutRequest,
  prepareAntiforgery,
  switchWorkspace as switchRequest,
  type AuthSession,
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const breakGlassLogin = useCallback(async (email: string, password: string) => {
    const value = await breakGlassLoginRequest(email, password);
    await prepareAntiforgery();
    setSession(value);
    setError(undefined);
  }, []);

  const clearWorkspaceQueries = useCallback(async () => {
    const detail: { tasks: Promise<unknown>[] } = { tasks: [] };
    window.dispatchEvent(new CustomEvent("convolab:workspace-changing", { detail }));
    await Promise.all(detail.tasks);
  }, []);

  const logout = useCallback(async () => {
    await clearWorkspaceQueries();
    await logoutRequest();
    setSession(undefined);
  ...
  const switchWorkspace = useCallback(
    async (id: string) => {
      await clearWorkspaceQueries();
      const value = await switchRequest(id);
      setSession(value);
  ...
  const value = useMemo(
    ...
    [session, loading, error, workspaceEpoch, login, breakGlassLogin, logout, switchWorkspace, retry],
  );
