...
const topics = {
  ...
  analytics: {
    ...
    summary: "Monitor workspace and environment telemetry, token usage, actual/estimated ZAR cost, quality gates, policy enforcement, and asynchronous safe exports.",
    capabilities: [
      "Track workspace and environment attribution with content-derived configuration revisions.",
      "Record safe event dimensions without copying raw prompts, secrets, or message bodies.",
      "Calculate actual billed cost, token-based ZAR estimates, or explicit unavailable states.",
      "Generate asynchronous, formula-injection-safe CSV exports up to 100k rows with retention policies.",
    ],
    workflow: [
      "Select the target environment and time range in the Analytics workspace.",
      "Filter by provider, model, capability, outcome, or configuration revision.",
      "Inspect usage, cost, budget, quality, governance, and performance metrics.",
      "Export event data or inspect correlation-grouped execution events.",
    ],
    endpoints: [
      "GET /api/workspaces/{id}/analytics/overview",
      "GET /api/workspaces/{id}/analytics/usage",
      "GET /api/workspaces/{id}/analytics/cost",
      "GET /api/workspaces/{id}/analytics/budget",
      "GET /api/workspaces/{id}/analytics/quality",
      "GET /api/workspaces/{id}/analytics/governance",
      "GET /api/workspaces/{id}/analytics/performance",
      "GET /api/workspaces/{id}/analytics/adoption",
      "GET /api/workspaces/{id}/analytics/events",
      "GET /api/workspaces/{id}/analytics/correlations/{correlationId}",
      "GET|POST /api/workspaces/{id}/analytics/exports",
    ],
  },
  settings: {
    title: "Studio Settings",
    summary: "Manage workspace-wide and environment-scoped configuration, effective value resolution, and audit change logs.",
    capabilities: [
      "Resolve effective hierarchy values from default, workspace, and environment scopes.",
      "Manage environment lifecycles: create, update, activate, suspend, and archive.",
      "Configure provider credentials, token budgets, retention policies, and UI preferences.",
      "Inspect complete configuration change audit history with before/after diffs.",
    ],
    workflow: [
      "Open Studio Settings and select Workspace or Environment scope.",
      "Review effective configuration values and inheritance status.",
      "Update settings keys with validated formats and comments.",
      "Review change logs to verify audit attribution.",
    ],
    endpoints: [
      "GET|POST /api/workspaces/{id}/environments",
      "GET|PATCH /api/workspaces/{id}/environments/{envId}",
      "POST /api/workspaces/{id}/environments/{envId}/{activate|suspend|archive|make-default|select}",
      "GET|PATCH|DELETE /api/workspaces/{id}/settings",
      "GET /api/workspaces/{id}/settings/effective",
      "GET /api/workspaces/{id}/settings/changes",
      "GET|PATCH|DELETE /api/workspaces/{id}/environments/{envId}/settings",
