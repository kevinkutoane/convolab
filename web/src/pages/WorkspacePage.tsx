...
function WorkspaceTab({ tab, workspaceId, onInvalidate }: { tab: Tab; workspaceId: string; onInvalidate: (key: string) => void }) {
  ...
  return <section className="panel settings-panel"><Settings /><div><h2>Workspace settings</h2><p>Environment promotion and organisation-wide sharing are intentionally deferred beyond alpha.12.</p><dl><div><dt>Organisation</dt><dd>{workspace.data?.organisationId}</dd></div><div><dt>Workspace ID</dt><dd>{workspace.data?.id}</dd></div></dl></div></section>;
}
