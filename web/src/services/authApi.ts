...
let antiforgeryRequestTokenPromise: Promise<string> | undefined;

export function getAntiforgeryRequestToken() {
  return antiforgeryRequestToken;
}

export function clearAntiforgeryRequestToken() {
  antiforgeryRequestToken = undefined;
}
...
export interface AuthenticationOptions {
  ...
  entraLoginAvailable: boolean;
  breakGlassAvailable: boolean;
  entraLoginPath: string;
}
...
export async function breakGlassLogin(email: string, password: string) {
  return request<AuthSession>("/api/auth/break-glass/login", {
    method: "POST",
    body: JSON.stringify({ email, password }),
  });
}
...
export async function getAuthenticationOptions() {
  return request<AuthenticationOptions>("/api/auth/options");
}
...
export async function prepareAntiforgery(force = false) {
  ...
  antiforgeryRequestTokenPromise = (async () => {
    const value = await request<{ token: string }>("/api/auth/antiforgery");
    antiforgeryRequestToken = value.token;
    return value.token;
  })();

  try {
    return await antiforgeryRequestTokenPromise;
  } finally {
    antiforgeryRequestTokenPromise = undefined;
  }
}

export async function logout() {
  await request("/api/auth/logout", { method: "POST" });
  clearAntiforgeryRequestToken();
}

export async function switchWorkspace(workspaceId: string) {
  return request<AuthSession>("/api/auth/workspace", {
    method: "POST",
...
export async function getServiceAccounts(id: string) {
  return request<ServiceAccount[]>(`/api/workspaces/${id}/service-accounts`);
}

export async function getAudit(id: string) {
  return request<AuditEvent[]>(`/api/workspaces/${id}/audit`);
}
...
export async function createServiceAccount(
  ...
  return request<{ credential: string }>(`/api/workspaces/${id}/service-accounts`, {
    method: "POST",
    body: JSON.stringify(input),
  });
}
