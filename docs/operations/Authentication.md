...
Operational evidence is available to Platform Administrators at `GET /api/operations/authentication`. It contains only break-glass enabled/available flags, aggregate state, last successful use, and a recent aggregate failure count. Safe mode does not block authentication, callbacks, logout, session validation, identity administration, or break glass.
