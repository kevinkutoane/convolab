...
All genuine failed Entra callbacks contribute to the 24-hour failure aggregate through one persisted audit/outbox path. Framework `RemoteFailure` records failures that occur before application validation, while an internal request-local marker suppresses a second record when `RejectAsync` already persisted the application rejection. Only bounded internal failure classifications are stored.
