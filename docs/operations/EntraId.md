...
Troubleshoot safe failure codes through structured audit/telemetry and correlation IDs. Framework failures before token validation, token-exchange/client-authentication failures, and application rejections persist one sanitized `Authentication.EntraLogin` failed audit plus its trusted Analytics outbox mapping. A request-local marker prevents `RemoteFailure` from duplicating application rejection evidence. Never log or persist raw exceptions, callback query values, tokens, authorization codes, state, nonce, issuer, subject, email, tenant IDs, or client-secret material, and never enable identity-model personally identifiable information logging.
