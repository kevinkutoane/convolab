...
public sealed class EvaluationStudioService : IEvaluationStudioService
{
        ...
        var trend = Enumerable.Range(0, 7)
            ...
            .Select(date =>
            {
                var dayRuns = runs
                    ...
                    .ToList();
                return new EvaluationDailyTrendDto(
                    date,
                    dayRuns.Count,
    ...
    public async Task<IReadOnlyList<EvaluationScorecardDto>> ListScorecardsAsync(
        CancellationToken cancellationToken = default)
    ...
    public async Task<EvaluationPreviewDto> PreviewAsync(
        EvaluationPreviewCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        var policy = new EvaluationPolicyDto(
    ...
    private async Task<EvaluationPolicyDto> GetScorecardPolicyAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        return new EvaluationPolicyDto(
    ...
    private static EvaluationRunDto Map(
    ...
    private static EvaluationPreviewDto Evaluate(
        double groundedness,
        double relevance,
        double safety,
        EvaluationPolicyDto policy)
    ...
    private static EvaluationGateDecisionDto Decision(string name, double score, double threshold)
        => new(name, score, threshold, score >= threshold ? "Passed" : "Failed");

    private static EvaluationMetricSummaryDto Summarize(
        string name,
        ...
        double threshold)
    {
        ...
        return new EvaluationMetricSummaryDto(
            name,
            values.Average(),
            values.Min(),
            values.Max(),
    ...
    private static void ValidatePreview(EvaluationPreviewCommand command)
    {
    ...
    private static EvaluationScorecardDto MapScorecard(EvaluationScorecardState scorecard)
        => new(
            scorecard.Id,
            scorecard.Name,
