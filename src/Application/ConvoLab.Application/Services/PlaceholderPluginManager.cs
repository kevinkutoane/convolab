...
public class PlaceholderPluginManager : IPluginManager {
    public Task<PluginId> RegisterPluginAsync(string name, string description, string version, string manifestUrl, CancellationToken cancellationToken = default) => Task.FromResult(new PluginId(Guid.NewGuid()));
