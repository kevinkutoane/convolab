...
public class PlaceholderTraceEngine : ITraceEngine {
    public Task<TraceId> StartTraceAsync(string operationName, CancellationToken cancellationToken = default) => Task.FromResult(TraceId.CreateUnique());
    ...
    public Task<Trace> GetTraceAsync(TraceId traceId, CancellationToken cancellationToken = default) => Task.FromResult(Trace.Start("Dummy", Guid.NewGuid()));
}
