...
public sealed class CapabilityPermissionMiddleware(RequestDelegate next)
{
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        if (!path.StartsWith("/api/", StringComparison.Ordinal) || path.StartsWith("/api/auth") || path.StartsWith("/api/audit") || path.StartsWith("/api/workspaces") || path.StartsWith("/api/organisations") || path.StartsWith("/api/platform") || path.StartsWith("/api/operations"))
